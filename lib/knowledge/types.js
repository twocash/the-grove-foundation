...
export function rowToDocument(row) {
  return {
    ...
    promptsExtracted: row.prompts_extracted || false,
    promptExtractionAt: row.prompt_extraction_at ? new Date(row.prompt_extraction_at) : null,
    promptExtractionCount: row.prompt_extraction_count || 0,
  };
}
