...
/**
 * engagement-machine-check: Verify XState machine is valid and has correct initial states
 */
async function executeEngagementMachineCheck(def) {
  try {
    const engagementModule = await import('../src/core/engagement/index.ts');
 ...
 * Returns a Promise for async checks
 */
async function executeCheck(def) {
  switch (def.type) {
    case 'json-exists':
    ...
    case 'engagement-machine-check':
      return executeEngagementMachineCheck(def)
    default:
...
export async function runChecks(config) {
  ...
  for (const checkDef of allChecks) {
    ...
    // Execute the check (await for async checks)
    const result = await executeCheck(checkDef)
