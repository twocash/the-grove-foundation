...
const STAGE_CONFIG: Record<PromptStage, { label: string; color: string; icon: string }> = {
  genesis: { label: 'Genesis', color: 'text-green-400', icon: 'eco' },
  exploration: { label: 'Exploration', color: 'text-blue-400', icon: 'explore' },
  synthesis: { label: 'Synthesis', color: 'text-purple-400', icon: 'hub' },
  advocacy: { label: 'Advocacy', color: 'text-amber-400', icon: 'campaign' },
};
              ...
              return (
                <button
                  ...
                  className={`
                    ...
                    ${isSelected
                      ? `bg-[var(--glass-elevated)] ${config.color} border border-[var(--glass-border)]`
                      : 'bg-[var(--glass-surface)] text-[var(--glass-text-muted)] border border-transparent'
                    }
                    ${isSuggested ? 'ring-1 ring-cyan-500/50' : ''}
        ...
        {suggestion && (
          <div className="p-3 rounded-lg bg-cyan-500/10 border border-cyan-500/20">
            <div className="flex items-start gap-2">
              <span className="material-symbols-outlined text-cyan-400 text-base mt-0.5">
                auto_awesome
              </span>
              <div className="flex-1">
                <p className="text-xs text-cyan-300 font-medium">Targeting Suggestion</p>
          ...
          return (
            <span
              ...
              className={`w-1.5 h-1.5 rounded-full ${
                isAvailable ? config.color.replace('text-', 'bg-') : 'bg-[var(--glass-border)]'
              }`}
