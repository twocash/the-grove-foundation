...
export interface ResearchAgentConfigDataResult extends CollectionDataResult<ResearchAgentConfigPayload> {
  ...
  draftConfigs: GroveObject<ResearchAgentConfigPayload>[];
  /** Archived configurations */
  archivedConfigs: GroveObject<ResearchAgentConfigPayload>[];
  /** Activate a config (archives current active, sets target to active) */
  activate: (id: string) => Promise<void>;
  /** Create a new version from an existing config */
  createVersion: (sourceId: string, changelog?: string) => Promise<GroveObject<ResearchAgentConfigPayload>>;
  /** Save changes to active config as new version (creates new record, archives old) */
  saveAndActivate: (
    currentConfig: GroveObject<ResearchAgentConfigPayload>,
    pendingChanges: Partial<ResearchAgentConfigPayload>
...
export function useResearchAgentConfigData(): ResearchAgentConfigDataResult {
    ...
    async (id: string) => {
      ...
      try {
        ...
        if (activeConfig && activeConfig.meta.id !== id) {
          ...
          console.log('[ResearchAgentConfigData] Archived previous active:', activeConfig.meta.id);
        }
        ...
        console.log('[ResearchAgentConfigData] Activated:', id);
  ...
  const createVersion = useCallback(
    async (sourceId: string, changelog?: string) => {
      ...
      const versionedConfig: GroveObject<ResearchAgentConfigPayload> = {
        meta: {
          id: generateUUID(),
          type: 'research-agent-config',
  ...
  const saveAndActivate = useCallback(
    async (
      ...
      const newPayload: ResearchAgentConfigPayload = {
      ...
      const newConfig: GroveObject<ResearchAgentConfigPayload> = {
        meta: {
          ...currentConfig.meta,
          id: generateUUID(),
          status: 'active',
      ...
      console.log('[ResearchAgentConfigData] Creating new version:', {
      ...
      console.log('[ResearchAgentConfigData] Archived old version:', currentConfig.meta.id);

      // Step 2: Create new active record (now safe - no other active exists)
      let created: GroveObject<ResearchAgentConfigPayload>;
      try {
        created = await groveData.create(newConfig);
      } catch (createError) {
        // CRITICAL: Rollback - restore old config to active (never leave with no active)
        console.error('[ResearchAgentConfigData] Create failed, rolling back archive:', createError);
        await groveData.update(currentConfig.meta.id, [
          { op: 'replace', path: '/meta/status', value: 'active' },
          { op: 'replace', path: '/meta/updatedAt', value: new Date().toISOString() },
        ]);
        console.log('[ResearchAgentConfigData] Rollback complete - restored old version to active');
